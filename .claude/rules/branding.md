...
### FORBIDDEN Colors — Use surface colors instead
...
| Widgets | 7 | `ls widget/*.js \| wc -l` |
| Languages | 5 | `ls website/src/locales/*.json \| wc -l` |

Update ALL HTML + ALL 5 locale files when numbers change.

## Validation

`node scripts/validate-design-tokens.cjs` — v2.3, 17 checks. Run BEFORE CSS/HTML/widget commits.

### ⛔ RECURRING TASK (every 15-20 sessions — NEVER DELETE)
Run validator + verify STALE_NUMBER_PATTERNS matches real numbers. If changed: update validator → fix ALL codebase occurrences.
History: 250.120 personas 40→38 | 250.124 MCP 182→203
