 ...
 *
 * Run: node --test test/widget.test.mjs
 */
...
import templates from '../scripts/voice-widget-templates.cjs';

const WIDGET_DIR = path.join(import.meta.dirname, '../widget');
const LANG_PATH = path.join(import.meta.dirname, '../website/voice-assistant/lang');
...
describe('Widget security audit', () => {
  ...
  test('at least 7 widget JS files exist', () => {
    assert.ok(widgetFiles.length >= 7, `Expected ≥7 widget files, found ${widgetFiles.length}`);
  });
