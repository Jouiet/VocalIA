 ...
 *
 * Run: node --test test/kb-provisioner.test.mjs
 */
...
import { generateInitialKB, provisionKB, SUPPORTED_LANGUAGES } from '../core/kb-provisioner.cjs';
...
describe('KB Provisioner provisionKB', () => {
  ...
  test('returns result object with expected shape', () => {
    ...
    cleanupDirs.push(path.join(import.meta.dirname, '../clients', tenantId));
  ...
  test('creates KB files for all 5 languages', () => {
    ...
    const kbDir = path.join(import.meta.dirname, '../clients', tenantId, 'knowledge_base');
  ...
  test('creates config.json with widget_features', () => {
    ...
    const configPath = path.join(import.meta.dirname, '../clients', tenantId, 'config.json');
