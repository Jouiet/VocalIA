...
class RateLimiter {
  constructor(options = {}) {
    ...
    this.maxEntries = options.maxEntries || 10000; // Max unique keys to prevent memory DoS
  ...
  check(key) {
    ...
    if (!entry) {
      // Prevent memory DoS from distributed IPs
      if (this.requests.size >= this.maxEntries) {
        return { allowed: false, remaining: 0, resetTime: now + this.windowMs };
      }
...
function requestSizeLimiter(maxBytes = 1024 * 1024) {
  return (req, res, next) => {
    // C5 fix: check Content-Length header first for early rejection
...
function validateUrl(urlString, allowedHosts = []) {
  try {
    ...
    // Block internal IPs (C4 fix: full RFC 1918 172.16.0.0/12 range)
    const ip = url.hostname;
    const is172Private = (() => {
      const m = ip.match(/^172\.(\d+)\./);
      return m && parseInt(m[1], 10) >= 16 && parseInt(m[1], 10) <= 31;
    })();
    if (
      ip === 'localhost' ||
      ip === '127.0.0.1' ||
