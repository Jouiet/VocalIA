...
- **B5 WebSocket URL Fix** (250.261): Widget used `wss://api.vocalia.ma:3007` (port blocked by firewall) instead of `wss://api.vocalia.ma/realtime` (Traefik path route). Traefik config was already correct (strip `/realtime` → forward to :3007 internally). Fix: widget URL changed. cloud_voice: BROKEN → FIXED (code-side, needs VPS restart to pull latest).
- **Next (CRITICAL PATH)**: 1) SSH VPS → refresh Google OAuth tokens 2) Create Stripe Products/Prices on dashboard.stripe.com 3) Set STRIPE_SECRET_KEY + STRIPE_PRICE_* on VPS .env 4) Configure Stripe webhook URL → api.vocalia.ma/webhook/stripe 5) VPS restart (pulls B5 WebSocket fix + all latest code) 6) First paying customer

*Last update: 01/03/2026 — Session 250.261: B5 WebSocket URL fix (`:3007` → `/realtime`). Admin Platform Health widget. 4 bugs fixed total. cloud_voice BROKEN→FIXED. Remaining: B1 OAuth refresh (VPS), B3 Stripe keys (VPS).*
