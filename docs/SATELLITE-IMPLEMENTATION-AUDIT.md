...
> **250.265** : **Traefik routing +5 paths** (`/oauth`, `/api/billing`, `/api/admin`, `/api/plugins`, `/api/widget`) — sans ces routes, les nouveaux endpoints étaient 404 depuis l'extérieur (impacte le plugin-connect OAuth depuis satellites). Vérifié: `GET /api/auth/plugin-authorize` → 302 redirect vers login.html. VPS 7/7 HEALTHY. **Deploy auto** GitHub Actions FTP → NindoHost confirmé (vocalia.ma = NindoHost, PAS Hostinger). GSAP bento-grid fix déployé LIVE (pricing page comparison cards étaient invisibles).
> **250.266** : PHPUnit coverage audit — 6 CMS plugins passent de 104→122 tests (220 assertions, 0 fail). Gaps comblés : OAuth Connect, SRI integrity, XSS escaping, HTTPS URL. Bug fix : lien download PrestaShop cassé (pointait vers `vocalia-prestashop.zip` inexistant). 6 ZIPs rebuild + validation structurelle (SRI, CDN, tenant, HTTPS). Commits `02496aa7` + `617cf17b`.
