...
> Document de référence factuel | 02/03/2026 (MAJ 250.264) | Sources: blog officiel, docs Perplexity, Fortune, Heise, reviews indépendantes
...
> **250.266** : PHPUnit coverage audit exhaustif — 6 gaps critiques trouvés et comblés (+18 tests). 122/122 PHPUnit pass, 220 assertions. Bug fix: lien download PS cassé (`vocalia-prestashop.zip` → `vocalia.zip`). 6 ZIPs rebuild + validation (SRI, CDN, HTTPS, structure). Commits `02496aa7` + `617cf17b`.
...
| — | **Cross-system integration audit** | **HAUTE** | Fiabilité ++ | Faible | EventBus subscribers + WebhookRouter | ✅ DONE (250.252) — 6 bugs: EventBus .emit→.publish, payload envelope, hardcoded localhost, missing await, sync-to-3a guard |
